...
Kontrak pintar ini, bernama `Marketplace.sol`, berfungsi sebagai implementasi dasar dari *Decentralized Marketplace* sederhana di jaringan Ethereum. [cite: 156] Tujuan utamanya adalah untuk memfasilitasi penjual menambahkan produk (`addProduct`) dan pembeli membeli produk (`buyProduct`) menggunakan ETH. [cite: 156]

Fitur inti:
1.  **Pendaftaran Produk:** Penjual dapat mendaftarkan produk dengan detail (nama, harga, stok).
2.  **Pembelian:** Pembeli mengirimkan ETH yang setara dengan harga produk. [cite: 156]
3.  **Transfer Dana Otomatis:** Setelah pembelian sukses, ETH yang dibayarkan akan langsung dikirimkan ke alamat penjual. [cite: 157, 191]
4.  **Penandaan Terjual:** Jika stok produk habis, status `isSold` akan diperbarui menjadi `true`. [cite: 157, 190]

**Catatan:** Sesuai ketentuan, kontrak ini tidak mencakup fitur *front-end*, *rating*, maupun fungsi penarikan saldo terpisah (`withdraw`) untuk menjaga kesederhanaan. [cite: 162]

## b) Cara Menjalankan dan Pengujian di Remix IDE
Berikut adalah langkah-langkah untuk *deploy* dan menguji fungsionalitas utama kontrak ini menggunakan Remix IDE: [cite: 174]
...
5.  **Pengujian `addProduct` (Simulasi Penjual - Akun 2):** [cite: 168]
...
6.  **Pengujian `buyProduct` (Simulasi Pembeli - Akun 3):** [cite: 169, 170]
...
7.  **Verifikasi Status Produk (Pembacaan State):** [cite: 171]
...
Implementasi kontrak ini dibuat dengan asumsi dan penyederhanaan berikut: [cite: 175]
...
5.  **Satuan Harga dalam Wei:** Harga produk yang dimasukkan saat memanggil fungsi `addProduct` harus dalam satuan **wei**, bukan ETH. Wei adalah unit terkecil dari Ether. Konversi: `1 ETH = 1.000.000.000.000.000.000 wei` (10^18 wei).
